...
| `bike_router/tools/ensure_precache.py` | Устарело: stub; используйте `precache_area` |
...
Результат появится в **`./data/cache/area_precache/...`**. В **`.env`** держите **`PRECACHE_AREA_BUILD_ON_STARTUP=false`**: предсборка полигона не выполняется в `warmup` API.
...
**Коридор и холодный старт:** `CORRIDOR_WARMUP_PREBUILD` по умолчанию **`false`**. При **`true`** при старте API один раз строит граф коридора вокруг **`START_*` / `END_*`** из `.env` (phase1). Это **не** основной сценарий ускорения и **не** заменяет area precache по полигону.
...
Устаревший флаг **`PRECACHE_AREA_BUILD_ON_STARTUP`** (по умолчанию **`false`**) не используется в штатном деплое: предсборка только **`precache_area`**. Пример полигона — **`bike_router/tools/precache_samara_core.wkt.example`**.
...
**Progressive `/alternatives/start`** (фоновый job + polling) — **опциональная** веб-надстройка для UX; корректность маршрутов и кэшей **не** зависит от неё. При нескольких workers память графа не разделяется — см. ограничение про один worker.
