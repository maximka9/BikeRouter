...
**Коридор и холодный старт:** `CORRIDOR_WARMUP_PREBUILD=true` (по умолчанию в коде) при старте API один раз строит граф коридора вокруг пары **`START_*` / `END_*`** из `.env` (обычно без спутниковой зелени — «phase1»). Это **не гарантирует**, что первый **реальный** POST совпадёт с этим bbox: другие точки на карте дают другой прямоугольник → снова Overpass на первом «чужом» запросе. Для предсказуемого демо держите тестовые точки рядом с `START/END` из `.env`, задайте **фиксированную зону** (`AREA_*` / `AREA_POLYGON_WKT`) или смиритесь с долгим первым расчётом в новом коридоре (см. таймауты прокси в разделе про деплой выше).
...
При **`PRECACHE_AREA_ENABLED=true`**, если ось-выровненный bbox коридора текущего запроса **целиком лежит внутри** полигона из **`PRECACHE_AREA_POLYGON_WKT`** и **`meta.json`** совпадает по fingerprint с текущими весами/зеленью, загружается готовый GraphML **без** обращения к Overpass. Запросы вне арены или при невалидном кэше идут по обычному corridor-pipeline.

**Рекомендуемый порядок:** (1) **`PRECACHE_AREA_BUILD_ON_STARTUP=false`** в `.env`; (2) отдельно выполнить сборку в тот же каталог данных, что у runtime (**`BIKE_ROUTER_BASE_DIR`**); (3) поднять API — в `warmup` только проверка и загрузка готовых файлов с диска. Включать **`PRECACHE_AREA_BUILD_ON_STARTUP=true`** имеет смысл лишь для отладки «собрать при первом старте контейнера».
...
**`PRECACHE_AREA_BUILD_ON_STARTUP`** по умолчанию **`false`**. Значение **`true`** заставляет при старте API собирать кэш, если нет `graph_base.graphml` (долго, не подходит для «сервер только читает готовое»). Пример полигона (Самара, lon lat) — **`bike_router/tools/precache_samara_core.wkt.example`**.
...
**Progressive `/alternatives/start`:** третий вариант («зелёный») при `green_enabled=true` может досчитываться в **фоновом потоке** (`threading.Thread` в `api.py`) с polling по `job_id`. Это упрощает UX при долгой зелёной фазе, но не превращает сервис в устойчивую многопроцессную очередь задач: при нескольких workers память и кэши графа не разделяются осмысленно — см. ограничение про один worker.
